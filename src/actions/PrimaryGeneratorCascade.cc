...
:fGamma(G4ParticleTable::GetParticleTable()->FindParticle("gamma")), MinHf(1e-5)
{
...
void PrimaryGeneratorCascade::Decay_UpDownward(G4Event *event, LevelScheme::iterator it, std::string mode){
	...
	if(it != fLevelScheme.end()){
		...
		// // peak all levels
		// G4cout << "peaking energies" << G4endl;
		// 	for(auto const &it : levels){
		// 		G4cout << it.energy << G4endl;
		// 	}
		// G4cout << "peaking ended" << G4endl;

		if (levels.size() != 0 )
		{
			LevelScheme::iterator itrEE = RandGen(levels);
			// check if the decay time is short enough for coincidence
			if(( (itrEE->Hf < MinHf) || (itrEE->is_gamma) )!= 0){
				if(is_gamma_e(itrEE)){	
					ParticleGun(event, itrEE->energy);
				} 
...
void PrimaryGeneratorCascade::ParticleGun(G4Event* event, G4double energy){
	...
	// gamma->SetMomentumDirection(mom);
